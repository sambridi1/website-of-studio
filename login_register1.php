...
{
    $query="SELECT * FROM `user_log` WHERE `email`='$_POST[email_username]' OR `username`='$_POST[email_username]'";
...
{
    $user_exist_query="SELECT * FROM `user_log` WHERE `username`='$_POST[username]' OR `email`='$_POST[email]'";
    ...
    else  #it will be executed if no one has taken username or email before
    {
        ...
        $query="INSERT INTO `user_log`(`full_name`, `username`, `email`, `password`) VALUES ('$_POST[fullname]','$_POST[username]','$_POST[email]','$password')";
